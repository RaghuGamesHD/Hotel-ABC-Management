...
(function($) { // Hide scope, no $ conflict
	...
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {
			picker: '<div class="calendars">' +
						'<div class="calendars-nav">{link:prev}{link:today}{link:next}</div>' +
                            '{months}' +
						    '{popup:start}' +
                                '<div class="calendars-ctrl">{link:clear}{link:close}</div>' +
                                '<div>{time}</div>' +
                            '{popup:end}' +
            ...
            time : '<div class="calendars-time">{hour} : {minute} : {Seconds}</div>',
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
		...
		show: function(elem) {
            ...
            plugin._generateTimePicker(elem);
		},
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		_generateContent: function(elem, inst) {
            ...
            // Generate time
            var time = '';
            var hour = '<input type="number" class="calendars-time-hour">';
            var minute = '<input type="number" class="calendars-time-minute">';
            var seconds = '<input type="number" class="calendars-time-seconds">';



            var time = this._prepare(inst.options.renderer.time, inst).replace(/\{time\}/, time);
        ...
        /** Generate time picker.
         @memberof CalendarsPicker
          */
        _generateTimePicker: function(elem, inst){
